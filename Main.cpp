...
private:
    void initWindow() {
        ...
        m_Window = glfwCreateWindow(m_Wdith, m_Height, "Vulkan", nullptr, nullptr);
    ...
    void pickPhysicalDevice() {
        ...
        if (m_PhysicalDevice == VK_NULL_HANDLE) {
            throw std::runtime_error("Found at least one GPU, but none were suitable");
        }
    }
    ...
    void createLogicalDevice() {
        ...
        std::set<unsigned int> uniqueQueueFamilies = { indicies.graphicsFamily.value(), indicies.presentFamily.value() };
    ...
    void createCommandBuffers() {
        ...
        allocInfo.commandPool = m_CommandPool;
        allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
        allocInfo.commandBufferCount = (unsigned int)m_CommandBuffers.size();
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, unsigned int imageIndex) {
        ...
        if (vkBeginCommandBuffer(commandBuffer, &beginInfo) != VK_SUCCESS) {
            throw std::runtime_error("Failed to being recoding command buffer");
        }
